...
void adaptedresize(cv::Mat& cv_img, int smallest_side, const bool &use_deformed_resize) {
    int cur_width = cv_img.cols;
    int cur_height = cv_img.rows;
    cv::Size dsize;
    if(use_deformed_resize)
        cv::resize(cv_img, cv_img, cv::Size(smallest_side,smallest_side),cv::INTER_NEAREST);
    else{
        if (cur_height <= cur_width) {
            double k = ((double)cur_height) / smallest_side;
            int new_size = (int) ceil(cur_width / k);
...
void DataTransformer<Dtype>::Transform(const cv::Mat& img,
  ...
  else if (do_resize_to_min_side_min && do_resize_to_min_side_max) 
  {
    //std::cout << min_side_min << " "<<min_side_max<<std::endl;
    CHECK(min_side_min >= crop_size);
    int min_side_length = 0; 
    if(phase_ == TRAIN)
      min_side_length = min_side_min + Rand(min_side_max - min_side_min + 1);
    else
      min_side_length = crop_size;
    adaptedresize(cv_img, min_side_length,false);
  ...
  for (int h = 0; h < height; ++h) {
	...
	template<typename Dtype>
		void DataTransformer<Dtype>::TransformImageSeg(const cv::Mat& cv_img, const cv::Mat& cv_label,Blob<Dtype>* transformed_img_blob, Blob<Dtype>* transformed_label_blob, int ignore_label) {
			// Check dimensions.
			const int img_channels = cv_img.channels();
